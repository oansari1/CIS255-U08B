...
// Dropdown list passes values ($size), Checkboxes are supposed to pass an array of checked values - I'm only getting the last item checked for now
if(isset($_POST["customerName"])) { $name = $_POST["customerName"]; } else { $name = ""; }
if(isset($_POST["size"])) { $size = $_POST["size"]; } else { $size = ""; }
if(isset($_POST["topping"])) { $topping = array($_POST["topping"]); } else { $topping = array("Cheese"); }
...
// Print topping if checked. Because only last item checked is ending up in the array, my total is not getting added properly
if(in_array("Pepperoni", $topping)) {$total += 0.99; print "<tr><td>Pepperoni</td><td>$0.99</td></tr>";}
if(in_array("Pumpkin", $topping)) {$total += 0.99; print "<tr><td>Pumpkin</td><td>$0.99</td></tr>";}
if(in_array("Escargot", $topping)) {$total += 1.99; print "<tr><td>Escargot</td><td>$1.99</td></tr>";}
if(in_array("Gerbils", $topping)) {$total += 8.99; print "<tr><td>Gerbils</td><td>$8.99</td></tr>";}
if(in_array("Cheese", $topping)) {print "<tr><td>Just Cheese</td></tr>";}

print "<tr><th>Total: </th><td>$ $total</td></tr></table></body>";
?>
</body>
</html>